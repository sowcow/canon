...
END{
  ...
  def feed_all # > 4 days
    my_feed WTP.all_pages, 'reborn/all_pages'
  end
  def feed_digha # 05:52:00?
    my_feed WTP.get.only!(DIGHA).all_pages, 'reborn/digha'
  end
  def feed_majhima # 00:20:10
    my_feed WTP.get.only!(MAJJHIMA).all_pages, 'reborn/majjhima'
  end
  def feed_4_nikayas # unknown
    my_feed WTP.get.only!(FOUR_NIKAYAS).all_pages, 'reborn/four_nikayas'
  end  
